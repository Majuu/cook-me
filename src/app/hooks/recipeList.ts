...
export const useRecipeList = (
  isAllRecipesScreen: boolean,
  filterByCategory: RecipeCategories | undefined,
  filterByName: string,
): Recipe[] => {
  ...
  const [listFilteredByCategory, setListFilteredByCategory] = useState<
    Recipe[]
  >([]);
  const [listFilteredByCategoryAndName, setListFilteredByCategoryAndName] =
    useState<Recipe[]>([]);

  const allRecipes = useSelector(
    (state: RootState) => state.recipes.recipeList,
  );
  const favouriteRecipes = useSelector(
    (state: RootState) => state.recipes.favouritesRecipes,
  );

  useEffect(() => {
    setCurrentRecipeList(isAllRecipesScreen ? allRecipes : favouriteRecipes);
    setListFilteredByCategory(
  ...
  useEffect(() => {
    if (filterByCategory && filterByCategory !== RecipeCategories.NONE) {
      const categoryFilter = currentRecipeList.filter(
  ...
  useEffect(() => {
    if (filterByName.length) {
      const nameFilter = listFilteredByCategory.filter(item => {
        return item.title.toLowerCase().includes(filterByName.toLowerCase());
      });
